...
3. 让过程观察序列依次返回带战斗 HUD 的暂停帧、播放后的普通帧、真实结果帧，断言点击播放并最终返回 `settled`。
4. 让播放后的立即截图仍为暂停，断言继续观察并在后续结果帧结算；干员选择界面的相似灰色画面不得触发播放。
...
1. 添加已校准的结果标题、战斗 HUD 与暂停标题 ROI；导出其判断函数供测试使用。
2. 让 `observeMaaScreen` 和 `observeMaaBattle` 都只接受“已识别星标且命中结果标题”的结算。
3. 保存带战斗 HUD 的暂停帧后点击一次播放并立即复查。复查仍暂停时继续常规采样，直到出现非暂停战斗帧后才允许再次点击。
4. 不新增依赖、配置项或 CLI 参数。
