# 文档索引

1. [总体架构](architecture.md)：v2 模块边界、依赖方向和生成流程。
2. [算法边界](algorithm-boundary.md)：评分能力与不能声称的结论。
3. [语料驱动引擎](corpus-generator.md)：模型构建、候选搜索和反馈闭环。
4. [MAA Copilot 导出契约](maa-copilot-export-contract.md)：协议字段与必要测试。
5. [数据格式](data-format.md)：`MapData`、`StageFacts`、`EngineResult` 和 `BattleScript`。
6. [CLI 与 GUI](cli-design.md)：命令、接口和本地目录。
7. [PRTS.Map 适配器](prts-map-adapter.md)：输入解析和地图规范化。
8. [MAA 干员导出](maa-operator-export.md)：玩家干员库格式。
9. [实测关卡](test-levels.md)：训练集外验收关卡。

仓库内文档只描述 v2。v1 战斗实现及其历史文档通过 GitHub `v1.0.0-alpha` 标签查看。
