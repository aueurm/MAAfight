...
7. 点击 `验证脚本并进入演习`。支持的关卡会自动导航；若提示无法导航，请手动打开所选关卡详情页后重试。

这一步会：

- 先验证当前 JSON。
- 自动导航到目标关卡并确认详情页；使用手动入口时，请确保关卡与 GUI 中所选关卡一致。
- 如果代理指挥开启，就先取消代理指挥。
- 点击 `演习`，进入编队页。
...
MAA 6.17.5 的 `Fight times=0` 会跳过整个任务。演习入口改用经过检查的 MAA `Custom` 导航任务，停在所选关卡详情页后再验证演习按钮；无法确认安全路径时提示手动进入。
...
- 自动导航范围取决于本机 MAA 资源；无法导航的关卡仍可从目标详情页手动进入演习。
